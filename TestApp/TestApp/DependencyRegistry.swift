...
class DependencyRegistry {
    ...
    func prepare(viewController: NavigationExampleViewController) {
        let presenter = NavigationExamplePresenter()
        let vc = SectionTableViewController()
        viewController.inject(presenter: presenter, sectionController: vc)
    }
